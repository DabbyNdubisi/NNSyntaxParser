...
class ParseTrainer {
    ...
    init(serializer: ModelSerializer, explorationEpochThreshold: Int, explorationProbability: Float, featureProvider: TransitionFeatureProvider, model: ParserModel) {
    ...
    func parse(sentence: String) -> Parse {
        guard let rootPrefixRange = sentence.range(of: UDReader.rootPrefix) else {
            return parse(sentence: "\(UDReader.rootPrefix) \(sentence)")
        }
        ...
        var state = ParserAutomata(
            rootToken: rootToken,
            buffer: tagger.tags(in: range, unit: .word, scheme: .lexicalClass, options: [.omitWhitespace]).enumerated().map({ Token(i: $0.offset + 1, sentenceRange: $0.element.1, posTag: POSTag(nlTag: $0.element.0!)! ) }))
        while !state.isTerminal  {
            ...
            state.apply(transition: bestGuess)
        }
    ...
    func test(examples: [ParseExample]) -> Float {
        ...
        for example in examples {
            let answer = parse(sentence: example.sentence)
    ...
    func fasterTrain(examples: [ParseExample], batchSize: Int, startEpoch: Int = 1, epochs: Int) {
        ...
        for epoch in startEpoch...epochs {
            ...
            try! serializer.save(model: model, to: ParseTrainer.savedModelName(epoch: epoch))
    ...
    @discardableResult private func updateModel(optimizer: Adam<ParserModel>, features: EmbeddingInput, correct: Transition) -> Tensor<Float> {
    ...
    @discardableResult private func updateModel(optimizer: Adam<ParserModel>, batch: TransitionBatch) -> Tensor<Float> {
        let (loss, grad) = model.valueWithGradient { (model: ParserModel) -> Tensor<Float> in
